...
$ID_Produto = $nome = $preco = $imagens  = $desconto  = $Descricao = $Categoria_ID_Categoria = "";
$ID_Produto_err = $preco_err = $nome_err = $imagens_err = $desconto_err  = $Descricao_err  = $Categoria_ID_Categoria_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["imagens"]))){
        $imagens_err = "Por favor insira um nome.";

    } else {
        $sql = "SELECT ID_Produto FROM produto Where imagens = ?";

        if ($stmt = mysqli_prepare($con, $sql)) {
            mysqli_stmt_bind_param($stmt, "s", $param_imagens);
            //set parameters
            $param_imagens = trim($_POST["imagens"]);
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if (mysqli_stmt_num_rows($stmt) == 1) {
                    $imagens_err = "This login is already taken.";
                } else {
    ...
    if(empty(trim($_POST["preco"]))){
        $preco_err = "Por favor insira um nome.";

    } else {
        $sql = "SELECT ID_Produto FROM produto Where preco = ?";

        if ($stmt = mysqli_prepare($con, $sql)) {
            mysqli_stmt_bind_param($stmt, "s", $param_preco);
            //set parameters
            $param_preco = trim($_POST["preco"]);
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if (mysqli_stmt_num_rows($stmt) == 1) {
                    $preco_err = "This login is already taken.";
                } else {
    ...
    if(empty(trim($_POST["Descricao"]))){
        $Descricao_err = "Por favor insira um nome.";

    } else {
        $sql = "SELECT ID_Produto FROM produto Where Descricao = ?";

        if ($stmt = mysqli_prepare($con, $sql)) {
            mysqli_stmt_bind_param($stmt, "s", $param_Descricao);
            //set parameters
            $param_Descricao = trim($_POST["Descricao"]);
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if (mysqli_stmt_num_rows($stmt) == 1) {
                    $Descricao_err = "This login is already taken.";
                } else {
